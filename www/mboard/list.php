...
?>
	<li>
		<span class="col1"><?=$number?></span>		
			<?php 
				$view_url = "view.php?table=$table&num=$num&page=$page"; 
			?>
		<span class="col2">
			<a href="<?=$view_url?>">
        <?php
...
<?php
	if ($total_page>=2 && $page >= 2) {
		$new_page = $page-1;
		echo "<li><a href='list.php?table=$table&page=$new_page'>◀ </a> </li>";
	}		
	else 
		echo "<li>&nbsp;</li>";
  ...
  for ($i=1; $i<=$total_page; $i++) {
    if ($page == $i)     // 현재 페이지 번호 링크 안함
      ...
      echo "<li> <a href='list.php?table=$table&page=$i'> $i </a> <li>";
    }
...
<ul class="buttons">
		<?php
			$list_url = "list.php?table=$table&page=$page";
		?>
	<li><button onclick="location.href='<?=$list_url?>'">목록</button></li>
		<?php
			if($userid)	
				$onclick = "location.href='form.php?table=$table'";
			else
